...
def create_synthetic_dataset(
    ...
    video_dir = root / "videos" / "observation.images.ego" / "chunk-000"
    ...
    (root / "meta" / "info.json").write_text(
        json.dumps({"fps": 30.0, "features": features}),
        encoding="utf-8",
    )
    pq.write_table(
        pa.table({"task": ["synthetic hand motion"], "task_index": [0]}),
        ...
        pa.table(
            {
                ...
                "tasks": [["synthetic hand motion"] for _ in range(episodes)],
                ...
                "videos/observation.images.ego/chunk_index": np.zeros(
                    episodes,
                    dtype=np.int64,
                ),
                "videos/observation.images.ego/file_index": np.zeros(
                    episodes,
                    dtype=np.int64,
                ),
                "videos/observation.images.ego/from_timestamp": (
                    np.arange(episodes) * frames / 30.0
                ),
                "videos/observation.images.ego/to_timestamp": (
                    (np.arange(episodes) + 1) * frames / 30.0
                ),
            }
        ),
    ...
    extrinsics = np.broadcast_to(np.eye(4), (total_frames, 4, 4)).copy()
    local_x = np.tile(np.linspace(0.0, 0.1, frames), episodes)
    left = np.column_stack(
        [local_x, np.zeros(total_frames), np.ones(total_frames)]
    )
    ...
    frame_table = pa.table(
        {
            ...
            "state_mask": _list_array(
                np.ones((total_frames, 2), dtype=bool)
            ),
            "observation.state": _list_array(state),
            "fov": _list_array(
                np.broadcast_to([1.0, 0.7], (total_frames, 2))
            ),
            "intrinsics": _list_array(
                np.broadcast_to(np.eye(3).reshape(9), (total_frames, 9))
            ),
            "extrinsics_w2c": _list_array(
                extrinsics.reshape(total_frames, 16)
            ),
            "left_transl_world": _list_array(left),
            "left_orient_world": _list_array(eye3.reshape(total_frames, 9)),
            "left_hand_pose": _list_array(eye15.reshape(total_frames, 135)),
            ...
            "right_transl_world": _list_array(right),
            "right_orient_world": _list_array(eye3.reshape(total_frames, 9)),
            "right_hand_pose": _list_array(eye15.reshape(total_frames, 135)),
    ...
    with av.open(str(video_path), mode="w") as container:
        ...
        for index in range(total_frames):
            ...
            start = (index * 4) % (pixels.shape[1] - 12)
