...
dl{display:grid;grid-template-columns:92px 1fr;margin:18px 0;border-top:1px solid var(--line)}dt,dd{padding:7px 0;margin:0;border-bottom:1px solid var(--line)}dt{color:var(--muted)}dd{font:12px ui-monospace,monospace}.actions,.media-tabs{display:flex;gap:7px;margin:13px 0}.claim{background:var(--ink);color:var(--card);border-color:var(--ink)}.choices{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin:13px 0}.choices button[data-decision=confirmed].selected{background:var(--red);color:#fff}.choices button[data-decision=false_positive].selected{background:var(--green);color:#fff}.choices button[data-decision=unsure].selected{background:var(--ink);color:#fff}label{display:grid;gap:6px;color:var(--muted)}textarea{min-height:80px;resize:vertical}.message{min-height:21px;margin-top:8px;color:var(--muted)}.message.error{color:var(--red)}
.gold{margin:16px 0;padding:14px;border:1px solid var(--line);background:#f2f2ec}.gold h3{font-size:13px;margin:0 0 8px}.gold p{margin:5px 0;color:var(--muted)}.gold .step{margin-top:14px;padding-top:13px;border-top:1px solid var(--line)}.gold .step[hidden]{display:none}.gold-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.gold-grid button.selected,.issue-row button.selected,.media-tabs button.active{background:var(--ink);color:var(--card)}.issue-row{display:grid;grid-template-columns:minmax(120px,1fr) repeat(3,88px);gap:5px;align-items:center;margin:6px 0}.issue-row span{font-size:12px}.check-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin:8px 0}.check{display:flex;grid-template-columns:auto 1fr;align-items:center;gap:7px;color:var(--ink)}.check input{width:auto}.fields{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.fields label{font-size:12px}.gold-lock{width:100%;margin-top:9px}.gold-lock.ready{border-color:var(--blue);color:var(--blue)}
@media(max-width:850px){header{position:static}.top{align-items:start;flex-direction:column}.case{grid-template-columns:1fr}main{padding:8px 14px}.identity{width:100%}.identity input{flex:1}.issue-row{grid-template-columns:1fr repeat(3,1fr)}.check-grid,.fields{grid-template-columns:1fr}}
...
const state={events:[],filter:'queue',kindFilter:'all',tokens:{},drafts:{},structured:savedStructured,me:null};
...
function decisionLabel(value,e){if(isGold(e))return {confirmed:'不合格 / 返工',false_positive:'合格 / 可用',unsure:'需仲裁'}[value]||value;return {confirmed:'真实问题',false_positive:'模型误报',unsure:'不确定'}[value]||value}
...
function goldPanel(e){if(!isGold(e))return '';const issues=e.metrics?.issue_codes||[];const labels=e.metrics?.gold_labels||[];const causes=e.metrics?.cause_options||[];return `<div class="gold" data-role="gold"><h3>第一步 · 只看原视频</h3><p>先判断这段数据本身能否用于训练，不要根据规则名猜答案。选择后锁定，才会显示 MANO 和规则证据。</p><div class="gold-grid">${[['pass','画面与动作可用'],['fail','数据本身不可用'],['unsure','仅凭原视频无法判断']].map(([v,l])=>`<button data-blind="${v}">${l}</button>`).join('')}</div><button class="gold-lock" data-action="reveal">锁定盲审，查看规则与 MANO</button><div class="step" data-role="evidence" hidden><h3>第二步 · 逐项核对规则</h3><div class="media-tabs"><button data-view="raw" class="active">原视频</button><button data-view="annotated" ${e.metrics?.annotated_clip_path?'':'disabled'}>MANO 叠加</button></div><div class="issue-list">${issues.map(code=>`<div class="issue-row" data-issue="${esc(code)}"><span>${esc(issueLabel(e,code))}</span>${[['confirmed','真错误'],['false_positive','误报'],['unsure','不确定']].map(([v,l])=>`<button data-issue-verdict="${v}">${l}</button>`).join('')}</div>`).join('')}</div><h3 class="step">看到了什么（可多选）</h3><div class="check-grid">${labels.map(item=>`<label class="check"><input type="checkbox" data-observation="${esc(item.code)}">${esc(item.label)}</label>`).join('')}</div><h3 class="step">主要原因（可多选）</h3><div class="check-grid">${causes.map(item=>`<label class="check"><input type="checkbox" data-cause="${esc(item.code)}">${esc(item.label)}</label>`).join('')}</div><div class="fields step"><label>首个坏点（秒，可空）<input type="number" min="0" step="0.01" data-field="first_bad_s"></label><label>最后坏点（秒，可空）<input type="number" min="0" step="0.01" data-field="last_bad_s"></label><label>置信度<select data-field="confidence"><option value="">请选择</option><option value="high">高</option><option value="medium">中</option><option value="low">低</option></select></label></div></div></div>`}
function card(e){const note=state.drafts[e.event_id]??e.note??'';const heading=isGold(e)?'Gold 盲审 · 先看原视频':(e.error_label||title(e.kind));return `<article class="case" data-id="${esc(e.event_id)}"><div><video controls preload="metadata" src="/api/review/media/${encodeURIComponent(e.event_id)}?view=${isGold(e)?'raw':'primary'}"></video></div><section><div class="head"><div><div class="eyebrow">${esc(e.video_id)}</div><h2 data-role="title">${esc(heading)}</h2></div><span class="duration">${Number(e.duration_s).toFixed(2)}s</span></div><span data-role="state" class="badge ${esc(e.state)}"></span><div class="taxonomy" data-role="taxonomy"><span data-role="category"></span><span data-role="severity" class="severity"></span><code>${esc(e.kind)}</code></div><dl><dt>${isGold(e)?'审核片段':'异常区间'}</dt><dd>${Number(e.start_s).toFixed(2)}–${Number(e.end_s).toFixed(2)}s</dd><dt>当前结论</dt><dd data-role="decision"></dd><dt>版本</dt><dd data-role="version"></dd></dl><div class="actions"></div>${goldPanel(e)}<div class="choices">${['confirmed','false_positive','unsure'].map(d=>`<button data-decision="${d}">${decisionLabel(d,e)}</button>`).join('')}</div><label>备注<textarea placeholder="写清画面内容、时间点和判断依据">${esc(note)}</textarea></label><div class="message"></div></section></article>`}
...
function goldReady(e){if(!isGold(e))return true;const d=detail(e),issues=e.metrics?.issue_codes||[];return Boolean(d.blind_quality&&d.evidence_revealed&&d.confidence&&issues.every(code=>d.issue_verdicts[code]))}
function updateGold(node,e,canEdit){if(!isGold(e))return;const d=detail(e),revealed=Boolean(d.evidence_revealed);node.querySelector('[data-role=evidence]').hidden=!revealed;node.querySelector('[data-role=taxonomy]').hidden=!revealed;node.querySelector('[data-role=title]').textContent=revealed?(e.error_label||title(e.kind)):'Gold 盲审 · 先看原视频';node.querySelectorAll('[data-blind]').forEach(b=>{b.classList.toggle('selected',b.dataset.blind===d.blind_quality);b.disabled=!canEdit||revealed});const reveal=node.querySelector('[data-action=reveal]');reveal.disabled=!canEdit||revealed||!d.blind_quality;reveal.textContent=revealed?'盲审已锁定':'锁定盲审，查看规则与 MANO';reveal.classList.toggle('ready',Boolean(d.blind_quality&&!revealed));node.querySelectorAll('[data-issue]').forEach(row=>row.querySelectorAll('[data-issue-verdict]').forEach(b=>{b.classList.toggle('selected',d.issue_verdicts[row.dataset.issue]===b.dataset.issueVerdict);b.disabled=!canEdit}));node.querySelectorAll('[data-observation]').forEach(x=>{x.checked=d.observed_labels.includes(x.dataset.observation);x.disabled=!canEdit});node.querySelectorAll('[data-cause]').forEach(x=>{x.checked=d.causes.includes(x.dataset.cause);x.disabled=!canEdit});node.querySelectorAll('[data-field]').forEach(x=>{if(document.activeElement!==x)x.value=d[x.dataset.field]??'';x.disabled=!canEdit});node.querySelectorAll('[data-decision]').forEach(b=>b.disabled=!canEdit||!goldReady(e))}
function bindGold(node,e,canEdit){if(!isGold(e))return;const d=detail(e),persist=()=>{saveStructured();updateGold(node,e,canEdit)};node.querySelectorAll('[data-blind]').forEach(b=>b.onclick=()=>{if(!d.evidence_revealed){d.blind_quality=b.dataset.blind;persist()}});node.querySelector('[data-action=reveal]').onclick=()=>{if(d.blind_quality){d.evidence_revealed=true;persist()}};node.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>{if(b.disabled)return;const video=node.querySelector('video'),position=video.currentTime||0;video.src=`/api/review/media/${encodeURIComponent(e.event_id)}?view=${b.dataset.view}`;video.load();video.onloadedmetadata=()=>{video.currentTime=Math.min(position,video.duration||position)};node.querySelectorAll('[data-view]').forEach(x=>x.classList.toggle('active',x===b))});node.querySelectorAll('[data-issue]').forEach(row=>row.querySelectorAll('[data-issue-verdict]').forEach(b=>b.onclick=()=>{d.issue_verdicts[row.dataset.issue]=b.dataset.issueVerdict;persist()}));node.querySelectorAll('[data-observation]').forEach(x=>x.onchange=()=>{d.observed_labels=x.checked?[...new Set([...d.observed_labels,x.dataset.observation])]:d.observed_labels.filter(v=>v!==x.dataset.observation);persist()});node.querySelectorAll('[data-cause]').forEach(x=>x.onchange=()=>{d.causes=x.checked?[...new Set([...d.causes,x.dataset.cause])]:d.causes.filter(v=>v!==x.dataset.cause);persist()});node.querySelectorAll('[data-field]').forEach(x=>x.onchange=()=>{d[x.dataset.field]=x.value;persist()});updateGold(node,e,canEdit)}
function bindCard(node,e){const id=e.event_id,textarea=node.querySelector('textarea');textarea.oninput=()=>state.drafts[id]=textarea.value;const claim=node.querySelector('[data-action=claim]');if(claim)claim.onclick=()=>act(node,claim,'领取中…',async()=>{const result=await api(`/api/review/events/${encodeURIComponent(id)}/claim`,{method:'POST',body:JSON.stringify({reviewer:who(),lease_seconds:900})});state.tokens[id]=result.claim_token;acceptEvent(result);await refreshStats()});const release=node.querySelector('[data-action=release]');if(release)release.onclick=()=>act(node,release,'释放中…',async()=>{const result=await api(`/api/review/events/${encodeURIComponent(id)}/release`,{method:'POST',body:JSON.stringify({reviewer:who(),claim_token:state.tokens[id]})});delete state.tokens[id];delete state.drafts[id];acceptEvent(result);await refreshStats()});node.querySelectorAll('[data-decision]').forEach(button=>button.onclick=()=>{if(!goldReady(e)){node.querySelector('.message').textContent='请先完成盲审、逐项规则判断和置信度';return}act(node,button,'提交中…',async()=>{const result=await api(`/api/review/events/${encodeURIComponent(id)}/decision`,{method:'POST',body:JSON.stringify({reviewer:who(),claim_token:state.tokens[id],decision:button.dataset.decision,note:textarea.value,expected_version:e.version,details:isGold(e)?detail(e):{}})});delete state.tokens[id];delete state.drafts[id];delete state.structured[id];saveStructured();acceptEvent(result);await refreshStats()})})}
function patchCard(node,e){const mine=e.state==='claimed'&&e.claimed_by===whoOrEmpty(),canDecide=mine&&state.tokens[e.event_id];const badge=node.querySelector('[data-role=state]');badge.className=`badge ${e.state}`;badge.textContent=e.state+(e.claimed_by?' · '+e.claimed_by:'');node.querySelector('[data-role=category]').textContent=e.category_label||e.category||'未分类';const severity=node.querySelector('[data-role=severity]');severity.className=`severity ${e.severity||'review'}`;severity.textContent=e.severity_label||e.severity||'需复核';node.querySelector('[data-role=decision]').textContent=e.decision||'—';node.querySelector('[data-role=version]').textContent=e.version;node.querySelector('.actions').innerHTML=`<button class="claim" data-action="claim" ${e.state==='reviewed'||(e.state==='claimed'&&!mine)?'disabled':''}>${mine?'续期':'领取'}</button>${mine?'<button data-action="release">释放</button>':''}`;node.querySelectorAll('[data-decision]').forEach(button=>{button.textContent=decisionLabel(button.dataset.decision,e);button.disabled=!canDecide||!goldReady(e);button.classList.toggle('selected',button.dataset.decision===e.decision)});const textarea=node.querySelector('textarea');textarea.disabled=!canDecide;if(document.activeElement!==textarea&&state.drafts[e.event_id]===undefined)textarea.value=e.note||'';bindCard(node,e);bindGold(node,e,canDecide)}
